...
gsep <- function( A, B, nz=min(ncol(A),ncol(B)) )
  {
    fnLib <- paste( R.home(), "lib/libRlapack.so", sep="/" )
    dyn.load( fnLib )
...
cc.scatter <- function( X, y )
  {
    ...
    print( "Computing the mean for each class..." )
    mc <- matrix( 0, p, k )
    for( i in 1:k )
      mc[,i] <- apply( X[,y == cc[i],drop=FALSE], 1, mean )

    print( "Computing within-cluster scatter..." )
    M <- X - mc[,as.integer(yf)]
    SW <- M %*% t(M) / n
    
    print( "Computing between-cluster scatter..." )
    SB <- X %*% t(X) / n - SW
...
lda.bases <- function( X, y, lambda=0.01 )
  {
